  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-4">
        <span className="text-xl font-extrabold tracking-tight">{data.client}</span>
        <span className="text-right text-xs font-semibold uppercase tracking-widest text-lime">
          {data.tag}
        </span>
      </div>

      <h3 className="mt-3 text-2xl font-bold leading-snug">{data.title}</h3>
      ...
      <div className="mt-8 grid grid-cols-3 gap-3">
        {data.metrics.map((m, i) => (
          ...
          >
            <div className="text-xl font-extrabold leading-none text-lime sm:text-2xl">
              {m.kind === "count" ? (
                <CountValue
                  value={m.value}
                  decimals={m.decimals}
  ...
  return (
    <section id="casos" className="bg-ink py-24 text-paper sm:py-32">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <h2 className="mt-4 max-w-3xl text-4xl font-extrabold leading-[1.05] tracking-tight sm:text-6xl">
